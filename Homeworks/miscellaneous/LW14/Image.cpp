...
// copy
Image::Image(const Image& objCopy) {
	// initialize object
	width = objCopy.getWidth();
	height = objCopy.getHeight();
	...
	for (size_t row = 0; row < height; ++row) {
		...
		for (size_t col = 0; col < width; ++col) {
			...
			image[col][row].setRed(objCopy.image[col][row].getRed());
			image[col][row].setGreen(objCopy.image[col][row].getGreen());
			image[col][row].setBlue(objCopy.image[col][row].getBlue());
...
Image& Image::operator=(const Image& objCopy) {
