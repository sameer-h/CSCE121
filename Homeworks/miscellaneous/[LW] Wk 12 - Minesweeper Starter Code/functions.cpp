...
int countClosedPositions(struct minePosition **mineMatrix, int nRows, int nCols) {
    int count = 0;
    for(int rws = 0; rws < nRows; rws++){
        for(int cls = 0; cls < nCols; cls++)
            if(!mineMatrix[rws][cls].open)
                count++;
    }
    return count;
}
...
int countNeighborMines(struct minePosition **mineMatrix, int nRows, int nCols, int row, int col) {
    ...
    for(int i = -1; i <= 1; i++)
        for(int y = -1; y <= 1; y++)
            if(row+y < nRows && row+y >= 0 && col+i < nCols && col+i >= 0)
                if(mineMatrix[row+y][col+i].neighborMines == MINE)
                    count++;
    return count;
}
...
int openPosition(struct minePosition **mineMatrix, int nRows, int nCols, int row, int col) {
    ...
    if(mineMatrix[row][col].neighborMines == 0){
        ...
        {
            for(int y = -1; y <= 1; y++)
                if(row+y < nRows && row+y >= 0 && col+x < nCols && col+x >= 0){
                    
                    if (!mineMatrix[row+y][col+x].open){
                        
                        openPosition(mineMatrix, nRows, nCols, row + y, col + x);
                    }
                }
        }
    }
